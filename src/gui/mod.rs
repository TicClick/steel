...
const CENTRAL_PANEL_INNER_MARGIN_X: i8 = 4;
...
pub fn png_to_rgba(bytes: &[u8]) -> Result<(Vec<u8>, (u32, u32)), png::DecodingError> {
    ...
    let mut reader = decoder.read_info().expect("Failed to unwrap PNG decoder");
    let mut buf = vec![0; reader.output_buffer_size().expect("Failed to unwrap PNG decoder's output buffer size")];
